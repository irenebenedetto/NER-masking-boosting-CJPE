...
def highlight_explanation(explanation: Explanation, k_top_type="k", value=10, sep=" "):
    ...
    tops = np.argsort(explanation.scores)[::-1][:k_top]
...
def get_most_relevant_sentences_ids(
    explanation, k_top_type="k", value=10, sort_by_order=True
):
    ...
    Args: explanation (Explanation)
          k_top_type (str): 'top_k' or 'perc'
          value (int): value for the top k or percentage (40 = 40%)
          sort_by_order (bool): sort sentence ids in ascending order - to have text explanations in the proper order
    Returns:
    ...
    if sort_by_order:
        ...
        top_sentences_ids = np.argsort(explanation.scores)[::-1][:k_top]
...
def ner_boosting(original_explanations, ner_explanations, boosting_parameter=5):
    ...
    for i in range(len(original_explanations)):
        w_scores = (
            original_explanations[i].scores
            + ner_explanations[i].scores
            * original_explanations[i].scores
            * boosting_parameter
        )
        e = Explanation(
            "",
            original_explanations[i].tokens,
            w_scores,
            "ner_gradient",
            original_explanations[i].target,
        )
